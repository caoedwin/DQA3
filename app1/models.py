...
class TestLog(models.Model):
    ...
    Tester = models.ForeignKey(to="UserInfo",to_field="Username",on_delete=models.SET_NULL ,null=True,)
    Comments = models.CharField('Comments', max_length=1000, default="", )
    Start_time = models.CharField('Start_time', max_length=26 , default="",)
    End_time = models.CharField('End_time',max_length=26, default="",)
    ...
    def __str__(self):
        ...
        return "%s+%s+%s+%s+%s+%s+%s+%s+%s"%(self.id,self.Project,self.Unit,self.Phase,self.Testitem,self.Start_time,self.End_time,self.Comments,self.Tester)
    # def getothers(self):
